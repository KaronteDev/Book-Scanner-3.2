...
from tkinter import (Tk, Toplevel, Frame, Label, Button, Canvas, filedialog, 
                    StringVar, DoubleVar, IntVar, Checkbutton, Entry, ttk, Scale, 
                    Listbox, SINGLE, END, HORIZONTAL, LEFT, RIGHT, BOTH, X, Y, 
                    TOP, BOTTOM, messagebox, Menu, Text)
...
# Minimal, self-contained replacement to restore parseability and provide App class hooks.
...
class App:
    """Main application class for the GeoDocs Scanner."""
    ...
    def setup_vars(self):
        ...
        # Document metadata
        self.title_var = StringVar(value="Documento")
        self.author_var = StringVar(value="")
        self.sig_var = StringVar(value="")
        self.tema_var = StringVar(value="")
        self.etiquetas_var = StringVar(value="")
        self.tipo_var = StringVar(value="")
        self.fecha_var = StringVar(value="")
        self.lugar_var = StringVar(value="")
        self.idioma_var = StringVar(value="")
        self.derechos_var = StringVar(value="")
        self.resumen_var = StringVar(value="")
        self.refbib_var = StringVar(value="")
        self.notas_var = StringVar(value="")
        
        # Processing settings
        self.dewarp_mode = StringVar(value="none")
        self.dewarp_strength = DoubleVar(value=0.2)
        self.contrast_mode = StringVar(value="clahe")
        self.final_scale = DoubleVar(value=1.0)
        self.ocr_enabled = IntVar(value=0)
        self.ocr_lang = StringVar(value="spa+eng")
        self.mesh_cols = IntVar(value=8)
        self.mesh_rows = IntVar(value=12)
        self.asymmetric_mesh = IntVar(value=0)
        self.capture_mode = StringVar(value="single")
        
        # Camera settings
        self.exposure_var = DoubleVar(value=-5.0)
        self.wb_var = DoubleVar(value=5000)
        
        # Output settings
        self.out_dir_var = StringVar(value=str(BASE_DIR))
        self.status_var = StringVar(value="Listo - Sin proyecto")
    ...
    def setup_ui(self):
        ...
        # Setup metadata panels
        self.setup_metadata_panels()
        ...
        # Setup status bar
        Label(root, textvariable=self.status_var).pack(side=BOTTOM, fill=X)
        
        # Setup additional panels
        self.setup_isad_panel()
        
        # Start preview loop
        self.root.after(30, self.update_preview_loop)

    def start_camera(self):
        cams = list_cameras_with_names()
        idx = cams[0][0] if cams else 0
    ...
    def draw_preview(self, bgr):
        h,w = bgr.shape[:2]
        cw = self.canvas_preview.winfo_width() or 640; ch = self.canvas_preview.winfo_height() or 360
        scale = min(cw/w, ch/h)
        rgb = cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)
        im = Image.fromarray(rgb).resize((int(w*scale), int(h*scale)))
        self.photo_preview = ImageTk.PhotoImage(im)
        self.canvas_preview.delete("all"); self.canvas_preview.create_image(cw//2, ch//2, image=self.photo_preview)

    def status(self, msg):
        self.status_var.set(msg)

def list_cameras_with_names(max_devices=6):
    names={}
    sysname = platform.system().lower()
    valid=[]
    for i in range(max_devices):
        try:
            cap=cv2.VideoCapture(i, cv2.CAP_DSHOW if sysname=="windows" else 0)
            ok,frame=cap.read(); cap.release()
            if ok: valid.append((i, f"Camera {i}"))
        except:
            pass
    return valid or [(0,"Camera 0")]

if __name__ == '__main__':
    ensure_schema = globals().get('ensure_schema')
    try:
        ensure_schema()
    except Exception:
        pass
...
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
import os, platform, subprocess, sqlite3, csv, json, requests, traceback, hashlib, sys
from datetime import datetime
from pathlib import Path
import cv2
import numpy as np
...
def detect_environment():
    ...
    if dev:
        tmp = GeoDocsAPI(); tmp.save_config(dev)
        ok, src = try_ping(tmp)
        if ok:
            return "development", dev
    if prod:
        tmp = GeoDocsAPI(); tmp.save_config(prod)
        ok, src = try_ping(tmp)
        if ok:
...
def ensure_schema():
    with db_connect() as con:
        ...
        cur.execute("""CREATE TABLE IF NOT EXISTS version_history (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          document_id INTEGER, page_id INTEGER, user TEXT, action TEXT, details TEXT, created_at TEXT
        )""")
        con.commit()
        ...
        for t,c in [("document","remote_id TEXT"),("document","last_sync TEXT"),("page","remote_id TEXT"),("page","remote_url TEXT"),("page","ocr_url TEXT"),("page","downloaded INTEGER DEFAULT 0")]:
            parts=c.split()
            ensure_column(t, parts[0], " ".join(parts[1:]))
        con.commit()

def list_cameras_with_names(max_devices=10):
    names={}; sysname=platform.system().lower()
    if sysname=="linux":
        for i in range(max_devices):
...
# compatibility helper functions that call GeoDocsAPI methods when bound to an instance
...
class APIMapperDialog(Toplevel):
    ...
    def edit_map(self, tree, section):
        ...
        self._edit_kv(tree, title=f"Editar mapeo ({section})", defaults=vals)
...
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
import os, platform, subprocess, sqlite3, csv, json, requests, traceback, shutil, tkinter as tk, hashlib
from tkinter import simpledialog
        if styled_text != text:
            # Update text
            self.current_page.delete("1.0", "end")
            self.current_page.insert("1.0", styled_text)
        else:
            self.status("No hubo cambios de estilo")
                    ...
                    try:
                        ...
                        messagebox.showerror("Error OCR", f"Error procesando imagen: {str(ex)}"); return
                    ...
                    with db_connect() as con:
                        try:
                            ...
                            messagebox.showerror("Error Base de Datos", f"Error guardando resultados OCR: {str(ex)}")
                            return
        ...
        m_style.add_command(label="Diff glosarios (A vs B)", command=lambda: diff_gloss_impl(self))
        m_style.add_command(label="Merge glosarios (preferir A)", command=lambda: merge_gloss_impl(self))
        menubar.add_cascade(label="Estilos/Glosarios", menu=m_style)
        m_help=Menu(menubar, tearoff=0)
        m_help.add_command(label="Ver tabla de abreviaturas (CSV/MD)", command=lambda: show_abbrev_info_impl(self))
    ...
    def open_api_config(self):
        ...
        e_url = Entry(dlg, width=50)
        e_url.insert(0, cfg.get("api_base_url",""))
        e_url.grid(row=0,column=1)
        
        e_key = Entry(dlg, width=50)
        e_key.insert(0, cfg.get("api_key",""))
        e_key.grid(row=1,column=1)
        
        e_to = Entry(dlg, width=10)
        e_to.insert(0, str(cfg.get("timeout_sec",20)))
        e_to.grid(row=2,column=1)
        
        v_tls = IntVar(value=1 if cfg.get("verify_tls",True) else 0)
        chk = Checkbutton(dlg, variable=v_tls)
        chk.grid(row=3,column=1,sticky="w")
        
        Button(dlg, text="Guardar",
               command=lambda:self._save_api_config(dlg,e_url,e_key,e_to,v_tls)
              ).grid(row=4,column=1,sticky="e",padx=6,pady=6)

    def _save_api_config(self, dlg, e_url, e_key, e_to, v_tls):
        try:
            cfg = {
                "api_base_url": e_url.get().strip(),
                "api_key": e_key.get().strip(),
                "verify_tls": bool(v_tls.get()),
                "timeout_sec": int(e_to.get().strip() or "20")
            }
            self.api.save_config(cfg)
            dlg.destroy()
            self.status("Config API guardada.")
        except Exception as ex:
    ...
    def setup_top_controls(self):
        ...
        Label(top, text="Modo:").pack(side=LEFT, padx=10)
        self.combo_mode = ttk.Combobox(top, state="readonly", width=10, values=["single", "double"])
    ...
    def setup_camera_controls(self):
        ...
        Label(ctrl, text="Exposición:").pack(side=LEFT, padx=5)
        Scale(ctrl, from_=-13, to=0, resolution=0.5, orient=HORIZONTAL,
              variable=self.exposure_var, command=self.apply_camera_props,
              length=200).pack(side=LEFT)
              
        Label(ctrl, text="Balance blancos K:").pack(side=LEFT, padx=5)
        Scale(ctrl, from_=2800, to=7500, resolution=100, orient=HORIZONTAL,
              variable=self.wb_var, command=self.apply_camera_props,
              length=250).pack(side=LEFT)

    def setup_main_content(self):
        mid = Frame(self.root)
        mid.pack(side=TOP, fill=BOTH, expand=True)
        
        # Left panel with preview and result
        left = Frame(mid)
        left.pack(side=LEFT, fill=BOTH, expand=True)
        
        self.canvas_preview = Canvas(left, bg="#111", width=700, height=400)
        self.canvas_preview.pack(side=TOP, fill=BOTH, expand=True, padx=5, pady=5)
        
        self.canvas_result = Canvas(left, bg="#111", width=700, height=400)
        self.canvas_result.pack(side=TOP, fill=BOTH, expand=True, padx=5, pady=5)
        
        # Right panel with page list
        right = Frame(mid)
        right.pack(side=RIGHT, fill=Y)
        
        Label(right, text="Páginas escaneadas").pack(side=TOP, pady=4)
        self.listbox = Listbox(right, selectmode=SINGLE, width=52, height=26)
        self.listbox.pack(side=TOP, padx=6, pady=4)

    def setup_metadata_panels(self):
        meta = Frame(self.root)
        meta.pack(side=TOP, fill=X, pady=4)
        
        Label(meta, text="Título:").pack(side=LEFT, padx=5)
        Entry(meta, textvariable=self.title_var, width=26).pack(side=LEFT, padx=5)
        
        Label(meta, text="Autor:").pack(side=LEFT, padx=5)
        Entry(meta, textvariable=self.author_var, width=18).pack(side=LEFT, padx=5)
        
        Label(meta, text="Signatura:").pack(side=LEFT, padx=5)
        Entry(meta, textvariable=self.sig_var, width=18).pack(side=LEFT, padx=5)

    def setup_processing_controls(self):
        proc = Frame(self.root)
        proc.pack(side=TOP, fill=X, pady=6)
        ...
        self.combo_dewarp.pack(side=LEFT, padx=5)
        
        Checkbutton(proc, text="OCR", variable=self.ocr_enabled).pack(side=LEFT, padx=10)
        Label(proc, text="OCR lang:").pack(side=LEFT, padx=5)
        Entry(proc, textvariable=self.ocr_lang, width=10).pack(side=LEFT, padx=2)

    def setup_output_controls(self):
        out = Frame(self.root)
        out.pack(side=TOP, fill=X, pady=6)
        
        Label(out, text="Salida:").pack(side=LEFT, padx=5)
        Entry(out, textvariable=self.out_dir_var, width=60).pack(side=LEFT, padx=5)
        Button(out, text="Cambiar carpeta...", command=self.choose_output_dir).pack(side=LEFT, padx=5)
        Button(out, text="Exportar PDF/A", command=self.export_pdf).pack(side=LEFT, padx=10)

    def setup_isad_panel(self):
        isad = Frame(self.root)
        isad.pack(side=TOP, fill=X, pady=6)
        
        Label(isad, text="ISAD(G) — Nivel:").pack(side=LEFT, padx=5)
        self.isad_nivel = ttk.Combobox(isad, state="readonly", width=16,
                                      values=["Fondo", "Subfondo", "Serie", "Subserie", "Unidad documental"])
        self.isad_nivel.set("Unidad documental")
        self.isad_nivel.pack(side=LEFT, padx=4)
        
        Label(isad, text="Productor:").pack(side=LEFT, padx=5)
        self.isad_productor = StringVar(value="")
        Entry(isad, textvariable=self.isad_productor, width=24).pack(side=LEFT, padx=4)
        
        Label(isad, text="Alcance y contenido:").pack(side=LEFT, padx=5)
        self.isad_alcance = StringVar(value="")
        Entry(isad, textvariable=self.isad_alcance, width=40).pack(side=LEFT, padx=4)
        
        Button(isad, text="Guardar ISAD(G)", command=self.save_isad).pack(side=LEFT, padx=10)

        # Tools bar for citations & report
        tools=Frame(self.root); tools.pack(side=TOP, fill=X, pady=6)
    ...
    def auto_detect_environment(self):
        ...
        if env and prof:
            try:
                ...
                messagebox.showerror("Autodetección", f"Error guardando configuración: {ex}")
        else:
    ...
    def show_citation_apa(self):
        with db_connect() as con:
            cur=con.cursor(); cur.execute("SELECT autor, fecha_text, title, lugar, signatura FROM document WHERE id=?", (self.doc_id,))
            r=cur.fetchone()
        if not r:
            messagebox.showwarning("Cita","Completa los metadatos y crea un proyecto."); return
        autor, fecha, titulo, lugar, signa = r
        cita=citation_apa(autor or "", fecha or "", titulo or "", None, None, signa or "", lugar or None)
    ...
    def show_citation_iso(self):
        ...
        if not r:
            messagebox.showwarning("Cita","Completa los metadatos y crea un proyecto."); return
        autor, fecha, titulo, lugar, signa = r
        cita=citation_iso690(autor or "", fecha or "", titulo or "", lugar or "", None, None, signa or "")
    ...
    def export_project_pdf(self):
        ...
        path = filedialog.asksaveasfilename(defaultextension=".pdf", filetypes=[("PDF","*.pdf")], initialfile="informe_proyecto.pdf")
        if not path: return
        try:
            ok = export_project_report(GLOBAL_DB, self.doc_id, Path(path), logo_path=BASE_DIR / "logo_geodocs.png")
            if ok:
                self.status(f"Informe exportado: {path}")
            else:
                messagebox.showerror("Informe","No fue posible generar el informe.")
        except Exception as ex:
            messagebox.showerror("Informe","Error generando informe: %s" % ex)
    ...
    def add_annotation(self):
        ...
        with db_connect() as con:
            con.execute("""INSERT INTO annotation (document_id, page_id, user, type, tags, body, target_region, created_at)
                           VALUES (?,?,?,?,?,?,?,datetime('now'))""",
                        (self.doc_id, page_id, getattr(self, 'anno_user', StringVar(value='user')).get() if hasattr(self, 'anno_user') else 'user',
                         getattr(self, 'anno_tipo', StringVar(value='note')).get() if hasattr(self, 'anno_tipo') else 'note',
                         getattr(self, 'anno_tags', StringVar(value='')).get() if hasattr(self, 'anno_tags') else '',
                         getattr(self, 'anno_body', StringVar(value='')).get() if hasattr(self, 'anno_body') else '',
                         getattr(self, 'anno_region', StringVar(value='')).get() if hasattr(self, 'anno_region') else ''))
            con.commit()
        add_version_entry(document_id=self.doc_id, page_id=page_id, action="add_annotation", details=(getattr(self, 'anno_body', StringVar(value='')).get() if hasattr(self, 'anno_body') else ''))
...
# (top-level duplicates removed; methods moved into class App)
    # ------- Exporters moved from top-level -------
        ...
        with db_connect() as con:
            con.execute("""INSERT INTO annotation (document_id, page_id, user, type, tags, body, target_region, latitude, longitude, toponimo_id, persona_id, created_at)
                        ...
                        (self.doc_id, page_id, getattr(self,'anno_user',StringVar(value='user')).get() if hasattr(self,'anno_user') else 'user',
                         getattr(self,'anno_tipo',StringVar(value='note')).get() if hasattr(self,'anno_tipo') else 'note',
                         getattr(self,'anno_tags',StringVar(value='')).get() if hasattr(self,'anno_tags') else '',
                         body, getattr(self,'anno_region',StringVar(value='')).get() if hasattr(self,'anno_region') else '',
                         lat_v, lon_v, topo_id, pers_id))
            con.commit()
    ...
    def open_web_map(self):
        import threading, webbrowser, subprocess, sys, time
    ...
    def search_persona(self):
        ...
        try:
            ...
            self.ent_pers_sel["values"] = vals
            if vals:
                self.ent_pers_sel.current(0)
        except Exception as ex:
            messagebox.showerror("Personas", str(ex))

    def save_geo_to_annotation(self):
        if not self.doc_id:
            messagebox.showwarning("Geo", "Crea un proyecto primero.")
            return
        ...
        if page_idx is not None:
            with db_connect() as con:
                cur = con.cursor()
                cur.execute("SELECT id FROM page WHERE document_id=? AND seq=?", (self.doc_id, page_idx))
                r = cur.fetchone()
                if r:
                    page_id = r[0]
        ...
        try:
            ...
            messagebox.showwarning("Geo", "Lat/Lon no válidos.")
            return
        ...
        with db_connect() as con:
            con.execute("""INSERT INTO annotation 
                          (document_id, page_id, user, type, tags, body, target_region, 
                           latitude, longitude, toponimo_id, persona_id, created_at)
                          VALUES (?,?,?,?,?,?,?,?,?,?,?,datetime('now'))""",
                       (self.doc_id, page_id, self.anno_user.get(), self.anno_tipo.get(),
                        self.anno_tags.get(), body, self.anno_region.get(),
                        lat_v, lon_v, topo_id, pers_id))
            con.commit()
            
        add_version_entry(document_id=self.doc_id, page_id=page_id,
                         action="add_annotation_geo", details=body)
        self.status("Geolocalización guardada en anotación.")
...
def do_ocr_current_page(self):
    ...
    try:
        # Set up Tesseract path before importing pytesseract
        import os
        import platform
        ...
        def find_tesseract_path():
            for path in TESSERACT_PATHS:
                if os.path.exists(path):
                    exe_name = 'tesseract.exe' if platform.system().lower() == 'windows' else 'tesseract'
        ...
        if not tesseract_path:
            raise RuntimeError("Tesseract no encontrado. Por favor instálalo primero en alguna de estas rutas:\n" + "\n".join(TESSERACT_PATHS))
            
        # Add Tesseract path to environment
        if tesseract_path not in os.environ['PATH'].split(os.pathsep):
            os.environ['PATH'] = os.pathsep.join([os.environ['PATH'], tesseract_path])

        # Set Tesseract data directory
        if platform.system().lower() == 'windows':
            os.environ['TESSDATA_PREFIX'] = os.path.join(tesseract_path, 'tessdata')
        
        # Now import pytesseract after environment is configured
        import pytesseract
        from PIL import Image
        
        # Configure tesseract command on Windows
        if platform.system().lower() == 'windows':
            pytesseract.pytesseract.tesseract_cmd = os.path.join(tesseract_path, 'tesseract.exe')
            
    except Exception as ex:
        messagebox.showerror("Error OCR", f"Error configurando OCR: {str(ex)}"); return
    ...
    try:
        conf = json.loads(cfg_path.read_text(encoding="utf-8")) if cfg_path.exists() else default_config
    except Exception as ex:
        self.status(f"Error leyendo configuración OCR, usando valores por defecto: {str(ex)}")
        conf = default_config
...
def diff_gloss_impl(app):
    """Module-level helper: compare two glossary rows in GLOBAL_DB by id. Expects 'app' instance for UI."""
    from tkinter import simpledialog, messagebox
    import sqlite3, json
    try:
        a = simpledialog.askinteger("Diff glosarios", "ID A:")
