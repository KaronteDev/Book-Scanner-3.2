#!/usr/bin/env python3
"""
Script para convertir messagebox en export_view.py - versión mejorada con parseo más robusto
"""
import re
...
def convert_file(filepath):
    ...
    while i < len(lines):
        ...
        if 'if' in line and 'messagebox.askyesno(' in line:
            # Reemplazar inline - usar un patrón que capture hasta el )  con mejor expansión
        ...
        # Procesar messagebox.showerror - mejor regex para capturar todo hasta el último paréntesis
...
if __name__ == '__main__':
    # Restaurar primero
    import subprocess
    subprocess.run(['git', 'checkout', 'gui/export_view.py'])
    
    f = Path('gui') / 'export_view.py'
    if f.exists():
        # Primero agregar import de Messagebox
        with open(f, 'r', encoding='utf-8') as file:
            content = file.read()
        
        # Agregar import si no existe
        if 'from ttkbootstrap.dialogs import Messagebox' not in content:
            ...
            with open(f, 'w', encoding='utf-8') as file:
                file.write(content)
            print("✓ Import de Messagebox agregado")
